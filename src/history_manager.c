...
int history_add(const char *name, const char *number, call_type_t type) {
  ...
  if (rc != SQLITE_OK) {
    printf("[HistoryManager] Failed to add log: %s\n", errmsg);
    sqlite3_free(errmsg);
    return -1;
  }
...
void history_clear(void) {
  ...
  if (rc != SQLITE_OK) {
    printf("[HistoryManager] Failed to clear history: %s\n", errmsg);
    sqlite3_free(errmsg);
  }
...
int history_load(void) {
  ...
  if (rc != SQLITE_OK) {
    printf("[HistoryManager] Failed to prepare select: %s\n",
           sqlite3_errmsg(db));
    return 0;
  }
  ...
  printf("[HistoryManager] Loaded %d history entries\n", g_history_count);

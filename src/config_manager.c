...
void config_manager_init(void) {
  ...
  mkdir(path, 0755);
  printf("[ConfigManager] Initialized, config dir: %s\n", path);
}
...
int config_load_app_settings(app_config_t *config) {
  ...
  if (fp) {
    ...
    fclose(fp);
    printf(
        "[ConfigManager] Loaded settings: Codec=%s, DefAcc=%d, AutoStart=%d\n",
  ...
  printf("[ConfigManager] Using defaults\n");
  return -1;
}
...
int config_save_app_settings(const app_config_t *config) {
  ...
  fp = fopen(path, "w");
  if (!fp) {
    printf("[ConfigManager] Failed to save settings\n");
    return -1;
  }
  ...
  printf("[ConfigManager] Saved settings\n");
  return 0;
}
...
int config_load_accounts(voip_account_t *accounts, int max_count) {
  ...
  fp = fopen(path, "r");
  if (!fp) {
    printf("[ConfigManager] No accounts file found\n");
    return 0;
  }
  ...
  while (count < max_count) {
    ...
    strcpy(acc->media_nat,
           ""); // Default to no ICE (prevents stunsrv errors if no STUN server)
    ...
    // We use a large buffer to read the line and then tokenize it to be more
    // robust? Or just extend fscanf. Let's extend fscanf but be careful.
    // Actually, fscanf with many fields is fragile.
    // Let's use fgets and manual parsing (strsep or strtok) for better
    ...
    if (fgets(line, sizeof(line), fp)) {
      ...
      if (strcmp(acc->server, "fanvi.com") == 0) {
        printf("[ConfigManager] Auto-correcting server: %s -> fanvil.com\n",
               acc->server);
        strcpy(acc->server, "fanvil.com");
        // Clear potentially unreachable private proxy
        if (acc->outbound_proxy[0] != '\0') {
          printf("[ConfigManager] Clearing outbound proxy: %s\n",
                 acc->outbound_proxy);
  ...
  fclose(fp);
  printf("[ConfigManager] Loaded %d account(s)\n", count);
  return count;
}
...
int config_save_accounts(const voip_account_t *accounts, int count) {
  ...
  fp = fopen(path, "w");
  if (!fp) {
    printf("[ConfigManager] Failed to save accounts\n");
    return -1;
  }
  ...
  fclose(fp);
  printf("[ConfigManager] Saved %d account(s)\n", count);
  return 0;
}

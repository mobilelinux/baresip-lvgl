...
enum dtmfmode { DTMFMODE_RTP_EVENT = 0, DTMFMODE_SIP_INFO, DTMFMODE_AUTO };
...
/** Jitter buffer type */
enum jbuf_type { JBUF_OFF, JBUF_FIXED, JBUF_ADAPTIVE };
...
int call_notify_sipfrag(struct call *call, uint16_t scode, const char *reason,
                        ...);
void call_set_handlers(struct call *call, call_event_h *eh, call_dtmf_h *dtmfh,
                       void *arg);
...
void call_get_media_estdir(struct call *call, enum sdp_dir *ap,
                           enum sdp_dir *vp);
...
int cparam_ua_decode(struct cparam_ua **prmp, const char *prm,
                     struct re_printf *pf);
...
int contact_add(struct contacts *contacts, struct contact **contactp,
                const struct pl *addr);
...
void contact_get_ldir(const struct contact *c, enum sdp_dir *aup,
                      enum sdp_dir *vidp);
...
int message_listen(struct message *message, message_recv_h *h, void *arg);
...
typedef int(ausrc_info_h)(const struct ausrc *ausrc, struct ausrc_prm *prm,
                          const char *device);
...
int ausrc_alloc(struct ausrc_st **stp, struct list *ausrcl, const char *name,
                struct ausrc_prm *prm, const char *device, ausrc_read_h *rh,
                ausrc_error_h *errh, void *arg);
int ausrc_info(struct list *ausrcl, const char *name, struct ausrc_prm *prm,
               const char *device);
...
int auplay_alloc(struct auplay_st **stp, struct list *auplayl, const char *name,
                 struct auplay_prm *prm, const char *device, auplay_write_h *wh,
                 void *arg);
...
typedef int(aufilt_encode_h)(struct aufilt_enc_st *st, struct auframe *af);

typedef int(aufilt_decupd_h)(struct aufilt_dec_st **stp, void **ctx,
                             const struct aufilt *af, struct aufilt_prm *prm,
                             const struct audio *au);
typedef int(aufilt_decode_h)(struct aufilt_dec_st *st, struct auframe *af);
...
typedef int(menc_media_h)(struct menc_media **mp, struct menc_sess *sess,
                          struct rtp_sock *rtp, struct udp_sock *rtpsock,
                          struct udp_sock *rtcpsock, const struct sa *raddr_rtp,
                          const struct sa *raddr_rtcp, struct sdp_media *sdpm,
...
const struct sa *net_laddr_for(const struct network *net, const struct sa *dst);
...
int play_file(struct play **playp, struct player *player, const char *filename,
              int repeat, const char *play_mod, const char *play_dev);
int play_tone(struct play **playp, struct player *player, struct mbuf *tone,
              uint32_t srate, uint8_t ch, int repeat, const char *play_mod,
              const char *play_dev);
...
int ua_connect(struct ua *ua, struct call **callp, const char *from_uri,
               const char *req_uri, enum vidmode vmode);
int ua_connect_dir(struct ua *ua, struct call **callp, const char *from_uri,
                   const char *req_uri, enum vidmode vmode, enum sdp_dir adir,
                   enum sdp_dir vdir);
void ua_hangup(struct ua *ua, struct call *call, uint16_t scode,
               const char *reason);
void ua_hangupf(struct ua *ua, struct call *call, uint16_t scode,
...
int ua_options_send(struct ua *ua, const char *uri, options_resp_h *resph,
                    void *arg);
...
int ua_enable_autoanswer(struct ua *ua, int32_t adelay, enum answer_method met);
int ua_disable_autoanswer(struct ua *ua, enum answer_method met);
int ua_call_alloc(struct call **callp, struct ua *ua, enum vidmode vidmode,
                  const struct sip_msg *msg, struct call *xcall,
                  const char *local_uri, bool use_rtp);
...
int cmd_register(struct commands *commands, const struct cmd *cmdv,
                 size_t cmdc);
...
const struct cmd *cmd_find_long(const struct commands *commands,
                                const char *name);
struct cmds *cmds_find(const struct commands *commands, const struct cmd *cmdv);
...
int vidsrc_alloc(struct vidsrc_st **stp, struct list *vidsrcl, const char *name,
                 struct vidsrc_prm *prm, const struct vidsz *size,
...
typedef int(vidisp_alloc_h)(struct vidisp_st **vp, const struct vidisp *vd,
                            struct vidisp_prm *prm, const char *dev,
                            vidisp_resize_h *resizeh, void *arg);
typedef int(vidisp_update_h)(struct vidisp_st *st, bool fullscreen, int orient,
...
int vidisp_alloc(struct vidisp_st **stp, struct list *vidispl, const char *name,
                 struct vidisp_prm *prm, const char *dev,
                 vidisp_resize_h *resizeh, void *arg);
...
typedef int(auenc_encode_h)(struct auenc_state *aes, bool *marker, uint8_t *buf,
...
typedef int(audec_decode_h)(struct audec_state *ads, int fmt, void *sampv,
                            ...
                            size_t len);
typedef int(audec_plc_h)(struct audec_state *ads, int fmt, void *sampv,
...
typedef int(videnc_packet_h)(bool marker, uint64_t rtp_ts, const uint8_t *hdr,
...
typedef int(vidfilt_encupd_h)(struct vidfilt_enc_st **stp, void **ctx,
                              ...
                              const struct video *vid);
typedef int(vidfilt_encode_h)(struct vidfilt_enc_st *st, struct vidframe *frame,
                              uint64_t *timestamp);

typedef int(vidfilt_decupd_h)(struct vidfilt_dec_st **stp, void **ctx,
                              const struct vidfilt *vf, struct vidfilt_prm *prm,
                              const struct video *vid);
typedef int(vidfilt_decode_h)(struct vidfilt_dec_st *st, struct vidframe *frame,
                              uint64_t *timestamp);
...
int vidfilt_enc_append(struct list *filtl, void **ctx, const struct vidfilt *vf,
                       struct vidfilt_prm *prm, const struct video *vid);
int vidfilt_dec_append(struct list *filtl, void **ctx, const struct vidfilt *vf,
                       struct vidfilt_prm *prm, const struct video *vid);
...
int audio_alloc(struct audio **ap, struct list *streaml,
                const struct stream_param *stream_prm, const struct config *cfg,
                struct account *acc, struct sdp_session *sdp_sess,
                const struct mnat *mnat, struct mnat_sess *mnat_sess,
                const struct menc *menc, struct menc_sess *menc_sess,
                uint32_t ptime, const struct list *aucodecl, bool offerer,
                audio_event_h *eventh, audio_level_h *levelh, audio_err_h *errh,
                void *arg);
...
int audio_encoder_set(struct audio *a, const struct aucodec *ac, int pt_tx,
                      const char *params);
int audio_decoder_set(struct audio *a, const struct aucodec *ac, int pt_rx,
                      const char *params);
...
int video_alloc(struct video **vp, struct list *streaml,
                const struct stream_param *stream_prm, const struct config *cfg,
                const struct account *acc, struct sdp_session *sdp_sess,
                const struct mnat *mnat, struct mnat_sess *mnat_sess,
                const struct menc *menc, struct menc_sess *menc_sess,
                const char *content, const struct list *vidcodecl,
                const struct list *vidfiltl, bool offerer, video_err_h *errh,
                void *arg);
int video_encoder_set(struct video *v, struct vidcodec *vc, int pt_tx,
                      const char *params);
...
typedef void(stream_rtcp_h)(struct stream *strm, struct rtcp_msg *msg,
                            void *arg);
...
void stream_set_session_handlers(struct stream *strm,
                                 ...
                                 stream_rtcp_h *rtcph, stream_error_h *errorh,
                                 void *arg);
struct stream *stream_lookup_mid(const struct list *streaml, const char *mid,
                                 size_t len);
...
typedef void(mnat_connected_h)(const struct sa *raddr1, const struct sa *raddr2,
                               void *arg);
...
typedef int(mnat_media_h)(struct mnat_media **mp, struct mnat_sess *sess,
                          ...
                          struct sdp_media *sdpm, mnat_connected_h *connh,
                          void *arg);

typedef int(mnat_update_h)(struct mnat_sess *sess);

typedef void(mnat_attr_h)(struct mnat_media *mm, const char *name,
                          const char *value);
...
int sdp_fingerprint_decode(const char *attr, struct pl *hash, uint8_t *md,
                           size_t *sz);
...
int bevent_call_emit(enum bevent_ev ev, struct call *call, const char *fmt,
                     ...);
...
int mediatrack_start_audio(struct media_track *media, struct list *ausrcl,
                           struct list *aufiltl);
...
int peerconnection_new(struct peer_connection **pcp,
                       ...
                       peerconnection_gather_h *gatherh, peerconnection_estab_h,
